...
def handle_connection(addr, player_id, controller_host, controller_port, debounce_time):
    ...
    try:
        ...
        # Send the player ID back to the ESP32
        # Using a special format: "PLAYER_ID:n" where n is 0-3
        player_message = f"PLAYER_ID:{player_id}"
        sock.send(player_message.encode('utf-8'))
        print(f"[Player {player_id+1}] Sent player ID back to ESP32: {player_id}")
        
        # Small delay to ensure message is received before processing data
        time.sleep(0.5)
